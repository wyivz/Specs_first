...
| 采集 | HTTP + Playwright + 平台 Adapter | 官网 / 视频 / 论坛 / 京东 / 淘宝天猫 |
| 模型 | Gemini + OpenAI | 文本脱水/OCR + 结构化仲裁 |
| 输出 | Obsidian Markdown + Dataview + CSV | 本地永久资产 |

```
Streamlit UI
    └── frontend/api_client.py（进程内调 FastAPI）
            └── backend/task_runner.py → Pipeline → Collectors → Obsidian
```
...
- [x] Adapter 插件注册表 `collectors/adapters/registry.py`
- [x] Obsidian + Dataview + CSV 导出
- [x] **单元测试 71 项** + GitHub Actions CI（`.github/workflows/test.yml`）
...
├── collectors/
│   ├── real.py / mock.py
│   ├── sources.py               # 官网/视频/论坛/电商采集
│   └── adapters/                # 京东/B站/YouTube/淘宝天猫
├── frontend/
│   ├── app.py                   # Streamlit UI
│   └── api_client.py            # 调 API
...
| P1 | 注册表接入 `sources.py` | 新平台扩展更省事（骨架已有） |
